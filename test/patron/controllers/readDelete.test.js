...
describe('controllers.patron.readDelete', () => {
  ...
  it('it should render patron/delete and pass one patron object', async () => {
    ...
    await patronController.readDelete(req, res);
    expect(res.render).to.have.been.calledWith('patron/delete', { dataValues: patron });
  });

  it('it should throw an error when a non-existent patron is requested for deletion', async () => {
    const res = mockResponse(),
          id = -1,
          req = mockRequest({ params: {id} }),
          patron = await patronService.readByPk(id);

    expect(await patronController.readDelete(req, res, err => err.message))
